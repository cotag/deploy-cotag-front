(function () {
    ...
    class CacheDatabase {
        ...
        'delete'(name) {
            ...
            return this.scope.caches.delete(`ngsw:composer:db:${name}`);
        }
        list() {
            return this.scope.caches.keys().then(keys => keys.filter(key => key.startsWith('ngsw:composer:db:')));
        }
        open(name) {
            if (!this.tables.has(name)) {
                const table = this.scope.caches.open(`ngsw:composer:db:${name}`)
    ...
    class AppVersion {
        constructor(scope, adapter, database, idle, manifest, manifestHash) {
            ...
            this.assetGroups = (manifest.assetGroups || []).map(config => {
                ...
                // group.
                const prefix = `ngsw:composer:${this.manifestHash}:assets`;
            ...
            this.dataGroups = (manifest.dataGroups || [])
                .map(config => new DataGroup(this.scope, this.adapter, config, this.database, `ngsw:composer:${config.version}:data`));
        ...
        deleteAllCaches() {
            return __awaiter$5(this, void 0, void 0, function* () {
                yield (yield this.scope.caches.keys())
                    .filter(key => key.startsWith('ngsw:composer:'))
